...
void sortPoints(point_t pts[], int size) {
	...
	for (start = 0;start < size - 1;start++) {
		...
		for (i = start + 1;i < size;i++) {
			...
			else if (pts[i].x == pts[min_index].x && pts[i].y < pts[min_index].y) {
				min_index = i;
			}
		}

		temp1 = pts[min_index];
		pts[min_index] = pts[start];
		pts[start] = temp1;
	}
}
...
int traceLines(point_t pts[], int size) {
	int diffy, diffx,i,length = 0;

	for(i = 0;i < size - 1;i++) {
		if (pts[i].x == pts[i+1].x) {
			diffy = pts[i+1].y - pts[i].y;
			length += diffy;
		}

		else if (pts[i].y == pts[i+1].y) {
			diffx = pts[i+1].x - pts[i].x;
			length += diffx;
		}
	}
